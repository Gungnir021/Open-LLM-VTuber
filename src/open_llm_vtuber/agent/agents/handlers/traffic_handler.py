...
from ..intent_detectors.traffic_detector import TrafficIntentDetector
...
class TrafficHandler(BaseHandler):
    ...
    async def handle(self, text: str, image_data: Optional[str] = None) -> str:
        ...
        Args:
            text: 用户输入文本
            image_data: 可选的图片数据（交通查询不需要）
            
        Returns:
            处理结果文本
        """
        # 使用交通意图检测器提取参数
        detector = TrafficIntentDetector()
        params = detector.extract_params(text)
        
        if not params.get("location"):
            return "抱歉，我无法确定您想查询哪个地点的交通状况。请提供具体的地点名称。"
        ...
        # 调用交通API
        traffic_result = self.tool_caller.call_tool("get_traffic_status", params)
        
        # 将工具结果添加到记忆中
        self.memory_manager.add_tool_result("get_traffic_status", traffic_result)
